...
def test_run_homr_geometry_only_invokes_cli_without_musicxml(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    preprocessed_input_path = tmp_path / "intermediate" / "preprocessed.png"
    ...
    def fake_run(
        ...
        *,
        ...
        assert command[:4] == [
            run_homr_module.sys.executable,
            "-m",
            "homr.main",
            "--geometry-only",
        ]
        assert command[4:6] == ["--geometry-json", str((output_dir / "geometry.json").resolve())]
        assert command[6:8] == [
            "--processed-image",
            str((output_dir / "homr_processed.png").resolve()),
        ]
        ...
        return subprocess.CompletedProcess(
            command,
            returncode=0,
            stdout="geometry stdout\n",
            stderr="geometry stderr\n",
        )

    monkeypatch.setattr(run_homr_module.subprocess, "run", fake_run)

    artifacts = run_homr_module.run_homr_geometry_only(
        preprocessed_input_path=preprocessed_input_path,
        output_dir=output_dir,
        logs_dir=logs_dir,
    )
    ...
    log_text = (logs_dir / "homr_geometry.log").read_text(encoding="utf-8")
    assert "geometry stdout" in log_text
    assert "geometry stderr" in log_text
